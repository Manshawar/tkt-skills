...
- 已有页重构:只对**用户选定的验收方向**打矩阵。交互模型变了 → 该页整页枚举;纯 token/主题换色 → 测切换+抽检,不要把 diff 扫到的每一页旧操作流都算进分母
- 关联路由一起算,但限于选定方向(`/test` 的「报告」链、`/test/report/full` 切换/返回)
- **入库范围** = 缺口且无不测原因;功能点覆盖率的分母是**选定方向**的清单行数(含拆行后的操作/直达),不是 snapshot 元素个数,也不是 diff 文件数
