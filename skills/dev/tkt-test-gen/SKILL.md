...
**验收方向由人定。** 有工作史先归纳;git `--stat` 只核对夹带、用来出选项。给出 3~5 项后**停下等选**。没选定禁止探测、禁止写矩阵、禁止读文件级 diff。整页重枚举只在用户勾了「该页交互模型变了」;全站换 token ≠ 旧操作流全测。
...
禁止整份 `git diff` 进上下文。顺序锁死:**归纳 → stat 出选项 → 停下 → 人选 → 再探测/再读文件 diff**。
...
输出 3~5 项。尚未选定 → **停止等回复**,禁止探测/写矩阵/`git diff -- <file>`。本轮已选定 → 选项仍写出来留底,不停第二次,进 1.2b/1.3。

```
这次改动本质: <一句话>
请选(可多选):
A. <主变化> — 测:<操作+可见结果> / 不测:<易跑偏旧交互> 【推荐】
B. <次要>
C. <stat 扫到但不是主变化> — 默认不推荐
D. 其他:你指定
```
