---
name: tkt-test-gen
description: "攒一批稳定 commit 后,生成白盒操作流 e2e 用例(像用户一样操作:导航/点击/输入/发送→断言可见结果),写回 e2e/ 的 spec + cases.json,半自动(草稿人确认后入库),进平台防回归。触发锚点 = 功能已稳定(页面结构近几次提交没再变 + 人已用 agent-browser 验过),不是刚做完。当用户说'沉淀一条用例防回归'、'这批改动统一做一轮 e2e'、'给这批 commit 生成测试'、'这个功能稳定了,固化成回归用例'、'攒一波做 e2e'时使用。动作:生成、写、沉淀、防回归、固化、攒批。对象:一批稳定 commit、功能、bug 修复、cases.json、spec。Triggers: 生成 e2e 用例、沉淀用例、防回归、攒批回归、固化用例、根据这批 commit。非触发: 刚做完功能想验一下/走一遍 → tkt-verify(agent-browser 截图人证);首次初始化 e2e → tkt-e2e-init。"
metadata:
  scope: project
---
...
**入口前提(稳定性)**:功能还没稳定(页面结构近几次提交还在变 / 人还没用 agent-browser 验过)→ 先走 `/tkt-verify` 日常验证,别在这里固化用例。本 skill 的输入是「一批已稳定的改动」,不是「刚做完的当次改动」。
