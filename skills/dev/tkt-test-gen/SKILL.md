...
**功能点矩阵是完成门,报告留证不是。** 探测到的每个可交互点必须拆成该拆的行(存在 / 操作 / 直达 / 条件展示),每行在 cases.json 有对应测试点,不测的写原因。已有页面重构/加交互 = 当新页面重枚举,不能只补一条基线。
...
- [ ] Step 1: 收集(读代码 + agent-browser 探测)⚠️ REQUIRED
  ...
  - [ ] 1.3 agent-browser snapshot -i + 读代码 URL/条件展示:产出功能点清单(存在/操作/直达/条件展示分列)
...
- 不测原因只允许:防递归(点冒烟/全量/分组)、纯装饰、已标注的环境数据依赖。禁止写「基线已经有这个按钮」。
...
先出示矩阵,再出示**入库范围**(缺口且无不测原因的草稿清单)。问:全部入库?删哪几条?改断言/分层?只看不写?

未确认禁止写回。不要把入库范围理解成「随便选几条喜欢的」——范围由矩阵推导,确认只是签字或删减。
