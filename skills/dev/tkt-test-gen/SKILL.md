...
- [ ] Step 1: 收集(读代码 + agent-browser 探测)⚠️ REQUIRED
  - [ ] 1.1 确定改动本质(有本轮对话/工作史 → 先归纳;没有才看 git 名单)
  - [ ] 1.2 对照 git 核对(防漏改/夹带),再出主验收点候选选项
...
先定「这次做了什么」,再探测。**禁止把整份 `git diff` 塞进上下文**。

**1.1 改动本质:历史优先,git 垫底**

1. **有本轮对话 / 刚做完的工作史** → 先用 1~3 句话归纳意图(用户怎么描述、改了哪类能力)。这是验收选项的主输入。文件多、stat 大,都不能盖过这句话。
2. **git 是核对,不是发现**:`git status --short` + `git diff --stat` + `git diff --cached --stat`(暂存/未暂存分开)。用来抓「史里没提的夹带改动」和「声称改了但工作区没有」。不要通读 diff。
3. **没有历史**(新会话、只丢了 commit/分支、用户说「按 diff 测」) → 才靠上面三份名单聚类。

选定方向之后,才对那几个路径 `git diff -- <file>` / 读文件。未选方向的文件不读。

归纳后回答三个问题:
1. **这次工作的主变化** —— 用户能感知的那一件事(来自历史;无历史才从文件名+stat 推)
2. **改动本质** —— 修 bug、加功能、调样式、改交互? **按本质聚类,不按文件清单摊开。**
3. **git 有没有夹带** —— stat 里行数很大但史没提的路径,单独列为「可能不是主验收」选项,默认不推荐

### 1.2b 验收方向 ⛔ BLOCKING

归纳完 **先出选项、停下问用户**,禁止直接开整页矩阵。选项 3~5 个,可多选:
...
- 有对话/工作史还先吞 git 名单(意图在历史上,文件清单会把主题做成整页重测)
