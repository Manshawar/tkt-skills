...
- **cases.json:group = 功能域,不是执行粒度**。group 是「页面 + 关联页面」组成的完整功能(如 `首页`、`测试平台`),一组 = 一次全量测试(交叉引用:页面 + 它操作到的关联页面/功能)。**冒烟/全量是执行粒度,不放进 group**:
  - **冒烟** = 本次提交/新增功能对应的 P0 用例(快、确定性),由平台 diff 驱动选例
  - **全量** = 跑完某个功能域 group 的全部用例(P0+P1)
  - priority 分层:P0 = 冒烟级(确定性断言,提交后必跑);P1 = 全量级(aiQuery/aiAssert 深层回归)
...
group / priority / desc: <group=功能域(页面+关联页面,不写冒烟/全量), priority=P0冒烟/P1全量, 对齐 cases.json>
```
...
- [ ] group 是功能域(页面+关联页面),非「冒烟/回归」;priority 区分 P0冒烟/P1全量
- [ ] 草稿经用户确认才写回
- [ ] 输出验证命令(固定范围 -g,不代跑全量)
