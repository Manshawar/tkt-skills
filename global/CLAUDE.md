...
- **agent-browser 是重操作**: 只在「验证」或「代码/日志/接口推不出的运行时事实」时启用; **能进仓库的(代码/样式/路由/签名/git 历史)一律不起浏览器, 书写阶段零浏览器**。要用时先跑 `agent-browser skills get core` 拿当版用法, 不凭记忆敲
...
- 无浏览器可用时: 用现有日志, **不为本次调试临时装 Playwright 顶替**; 沉淀回归用例走 `/tkt-e2e-init`(自带 Playwright+Midscene, 不属本条限制)
- **图片/截图读取**: 一律 `Read` + 本地绝对路径原生 image message，禁止 base64 直拼 message（base64 撑爆上下文且纯文本模型读不了）。按客户端分流：
  ...
  - 报 `Agent type 'vision-reader' not found`(会话早于该 agent 创建)时降级: `Agent(subagent_type:"general-purpose", model:"opus")`, prompt 内写明「只用 Read、禁 Bash、禁解码图片」
