...
- **图片/截图读取**: 一律 `Read` + 本地绝对路径原生 image message，禁止 base64 直拼 message。主模型多模态能直读准就主线程直接 Read；**主模型非多模态（纯文本，如 deepseek-v4-flash）或 Read 识别失败/读不准 → 不派子 agent**（实测子 agent 识别/UI 还原不如主模型直读），用 `tkt-cc-provider-switch` 开一个多模态主模型新窗口直读。
